...
def get_energy_model():
    model = VensimModel('energyTransition', wd='./models', model_file='RB_V25_ets_1_policy_modified_adaptive_extended_outcomes.vpm')
    
    model.outcomes = [TimeSeriesOutcome('cumulative carbon emissions'),
...
if __name__ == '__main__':
    ...
    policies = [Policy('adaptive policy', model_file='RB_V25_ets_1_policy_modified_adaptive_extended_outcomes.vpm'),
                Policy('no policy', model_file='RB_V25_ets_1_extended_outcomes.vpm')]
    
    with MultiprocessingEvaluator(model) as evaluator:
        results = evaluator.perform_experiments(6000, policies=policies)

    save_results(results, './data/6000_both_policies.tar.gz')