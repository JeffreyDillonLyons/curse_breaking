...
'''This one is forked from 'fork_18.04' and is intended to test the mean calculated from uhat as a stopping condition '''
...
def algorithm(P,species):
    ...
    global dick, old, candidates, poly, active, global_errors, no_nodes,step,means
    ...
    global_errors.append(sobol_error(means[-1]))
    ...
    while (global_errors[-1] > 0.01 or np.isnan(global_errors[-1])) and len(active)>0:
    ...
    print(f'GT_alpha:{gt_prey[0].round(4)}, GT_beta:{gt_prey[1].round(4)}, GT_delta:{gt_prey[2].round(4)}, GT_gamma:{gt_prey[3].round(4)}')
