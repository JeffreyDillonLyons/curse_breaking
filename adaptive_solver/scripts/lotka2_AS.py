...
def algorithm(P, species):
    ...
    df_indices_s1 = pd.DataFrame(
        columns=["alpha", "beta", "delta", "gamma"], dtype=object
    )
    ...
    print("-" * 20)
    print(
        f"ST_alpha:{st[0].round(10)}, ST_beta:{st[1].round(10)}, ST_delta:{st[2].round(10)}, ST_gamma:{st[3].round(10)}"
    )
    print(
        f"GT_alpha:{gt_prey[0].round(10)}, GT_beta:{gt_prey[1].round(10)}, GT_delta:{gt_prey[2].round(10)}, GT_gamma:{gt_prey[3].round(10)}"
    )
