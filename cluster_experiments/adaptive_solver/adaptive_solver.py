...
def generate_candidates(index_set, P):
    ...
    for candidate in candidates:
        ...
        if np.all(np.array(candidate) <= P):
...
def algorithm(P):
    ...
    while global_errors[-1] > 0.2 or np.isnan(global_errors[-1]):
        ...
        numpoly.savez(f'./data/poly/poly_{P}_{date_today}.npz',*poly)
    ...
    print('Congratulations, the algotithm has converged!')
